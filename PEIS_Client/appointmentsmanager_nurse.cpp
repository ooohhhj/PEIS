    ...
    ui(new Ui::AppointmentsManager_Nurse)
{
    ...
    connect(ClientSocket::instance(),&ClientSocket::OnPatientInfoResponce_Manager,this,&AppointmentsManager_Nurse::showAppointment);
}
...
void AppointmentsManager_Nurse::onViewReportClicked(const QModelIndex &index)
{
    if (!index.isValid()) {
        return;  // 如果索引无效，则返回
    }
...
void AppointmentsManager_Nurse::AppointmentManager(const QString &patientName, const QString &healthPackage, const QString &appointmentDate, const QString &appointmentStatus)
{
    // 创建对话框
...
void AppointmentsManager_Nurse::OncancelAppointmentButton(const QString &username, const QString &packageName, const QString &appointmentDate)
{
    QJsonObject obj;
...
void AppointmentsManager_Nurse::showAppointment(const QJsonArray &patientInfoArray)
{
    // 清空当前模型
